...
vid=cv2.VideoCapture("harder.mp4")
while vid.isOpened():
	framenum+=1
	_,ogimg=vid.read()
	if ogimg is None:
	...
	frame= cv2.Canny(finalw,50,150,L2gradient=True)			#Find the outlines of the yellow lines on the road
	...
	if lines is not None:
		...
		if len(slopepos)>0:
			...
			if mw>0.45:											#to exclude lines that have very low degree of inclination eg 1,2 degrees			
				...
				whiteslopes.append(mw)
				whiteintercepts.append(cw)
	else:
		if framenum!=1:
			if check==1:
				cv2.line(ogimg,(cordpos[0],cordpos[1]),(x2,y2),(0,0,255),9)
		...
		for cnt in contours2:
			...
			if M['m00']==area:		
				...
				if ((rect[1][1]*2.8)<rect[1][0]) or ((rect[1][0]*2.8)<rect[1][1]):	#checking whether its a line or not 
					...
					yellowslopes.append(m)
					yellowintercepts.append(c)	
				break	
